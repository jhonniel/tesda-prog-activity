<?php
        ...
        if(isset($_GET['type']) && $_GET ['type']=='restore' && isset
        ($_GET['id']) && $_GET['id']>0){
            $id=mysqli_real_escape_string($con, $_GET['id']);

           mysqli_query ($con,"UPDATE job_tbl SET status='1' WHERE id='$id'");
           alert("Product successfully restore!");
			 header("refresh:1; url=recycle.php?");

        }

        if(isset($_GET['type']) && $_GET ['type']=='delete' && isset
        ($_GET['id']) && $_GET['id']>0){
            $id=mysqli_real_escape_string($con, $_GET['id']);

           mysqli_query ($con,"DELETE FROM job_tbl WHERE id='$id'");
